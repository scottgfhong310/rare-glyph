...
public/apps/rare-glyph/icons/              # App icon：母版 SVG 深/淺＋favicon(.ico/.svg)＋PNG 16–512＋manifest
...
- **讀音兩欄**〔2026-09-19〕：`pinyin`（漢語拼音）／`zhuyin`（國語注音符號），**查得到才填、兩欄各自可空**；
...
- **備註欄 `note`**〔2026-09-19〕：策劃者的工作註記，**多行**（唯一允許換行的欄位，CRLF 正規化成 LF）。
  ...
  **欄位下方一排常用註記 chip**〔2026-09-19〕：點一下插進游標處（`與「」易混` 的游標落在「」裡、最後一顆是今天日期
  ——備註沒有 audit 欄位，要記「何時寫的」只能寫進內文）。⚠️ **清單是資料不是 UI 文案**（§6.1 兩條通道）
  ⇒ 不進 `locales`、不翻譯，改 `rare-glyph.js` 的 `NOTE_SNIPPETS` 陣列即可。
  ⚠️ 那幾條來自 owner 在**同領域**寫過的東西（`db_siddham` 的 21 筆理由）裡反覆出現的形狀，
  **不是「常用」的證據**（備註欄剛上線、0 筆有值）——**這個陣列本來就是要被改的**。

...
- **備考** — 複数行のキュレーターメモ（出典、紛らわしい字、未確認事項）。登録ファイルに残るだけで、**生成される span には入りません**。よく使う註記のチップ（本日の日付を含む）をクリックで挿入できます。
...
- **find** — 1 つの検索ボックスで `code` / `uni` / `ids` / `cbeta` / 読み / 備考 / ファイル名を横断フィルタ。**声調を区別しません**：`juan` で `juàn` が見つかります（声調記号だけを打った場合はその声調の字だけを列挙）。
...
buildSpan({ file, stem, ids, cbeta, code, uni, pinyin, zhuyin })   // → <span class="glyph" …> 文字列（字形登録）
buildCharSpan({ code, uni, pinyin, zhuyin })      // → <span data-code data-uni …>字</span>（字形なし登録）

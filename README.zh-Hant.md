...
管理**佛典與經典文獻中的罕見字／缺字**的策劃工具。它管理共用的缺字 SVG 語料、描述每個缺字（標準 Unicode IDS、CBETA 組字式、大正藏/CBETA 缺字碼、讀音、備註，以及若已存在的對應 Unicode 字），並產生可直接貼進 `markdown-library` / `markdown-reader` 文件的家族 `.glyph` `<span>` 標記。
...
- **讀音** — 記錄查得到的**拼音**（`lǒng`）與**國語注音**（`ㄌㄨㄥˇ`）；兩欄各自可空、自由文字（多音寫成 `luò / lào`）。附**收合式注音鍵盤**（37 符號＋4 聲調，點一下插進欄位）——一般輸入法打不出裸注音符號。
- **備註** — 多行的策劃註記（出處、與哪個字混用、還要再查什麼）。只留在登錄檔裡，**不會進到產生的 span**。附一排常用註記快速鍵（含今天日期）。
...
└─ public/
   ...
   │  ├─ glyphs.js                 # 登錄：window.RG_GLYPHS = [{file, ids, cbeta, code, uni, pinyin, zhuyin, note, timestamp}]
...
| POST | `/api/rare-glyph/registry` | 寫回 `glyphs.js`（先 `.bak`）— body `{ entries: [{file, ids, cbeta, code, uni, pinyin, zhuyin, note, timestamp}] }` |
...
buildSpan({ file, stem, ids, cbeta, code, uni, pinyin, zhuyin })   // → <span class="glyph" …> 字串（字形登錄）
buildCharSpan({ code, uni, pinyin, zhuyin })      // → <span data-code data-uni …>字</span>（無字形登錄）
...
// glyphs.js  →  window.RG_GLYPHS（每筆）
{
  ...
  "pinyin": "lǒng",         // 查得到的漢語拼音（多音寫成 "luò / lào"），可空
  "zhuyin": "ㄌㄨㄥˇ",       // 查得到的國語注音符號，可空
  "note":   "",             // 備註（多行的策劃註記；不會進到 span），可空

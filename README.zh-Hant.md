...
管理**佛典與經典文獻中的罕見字／缺字**的策劃工具。它管理共用的缺字 SVG 語料、描述每個缺字（標準 Unicode IDS、CBETA 組字式、大正藏/CBETA 缺字碼、讀音，以及若已存在的對應 Unicode 字），並產生可直接貼進 `markdown-library` / `markdown-reader` 文件的家族 `.glyph` `<span>` 標記。
...
- **讀音** — 記錄查得到的**拼音**（`lǒng`）與**國語注音**（`ㄌㄨㄥˇ`）；兩欄各自可空、自由文字（多音寫成 `luò / lào`）。
...
- **find** — 單一搜尋框跨 `code` / `uni` / `ids` / `cbeta` / 讀音 / 檔名不分欄位過濾。
...
└─ public/
   ...
   │  ├─ glyphs.js                 # 登錄：window.RG_GLYPHS = [{file, ids, cbeta, code, uni, pinyin, zhuyin, timestamp}]
...
| POST | `/api/rare-glyph/registry` | 寫回 `glyphs.js`（先 `.bak`）— body `{ entries: [{file, ids, cbeta, code, uni, pinyin, zhuyin, timestamp}] }` |

...
- **Readings** — record the **Pinyin** (`lǒng`) and **Zhuyin / Bopomofo** (`ㄌㄨㄥˇ`) you were able to look up; both optional and free-form (multiple readings as `luò / lào`).
...
- **Find** — one search box filters the list across `code` / `uni` / `ids` / `cbeta` / readings / filename (field-agnostic substring).
...
└─ public/
   ...
   │  ├─ glyphs.js                 # registry: window.RG_GLYPHS = [{file, ids, cbeta, code, uni, pinyin, zhuyin, timestamp}]
...
| POST | `/api/rare-glyph/registry` | Write back `glyphs.js` (`.bak` first) — body `{ entries: [{file, ids, cbeta, code, uni, pinyin, zhuyin, timestamp}] }` |

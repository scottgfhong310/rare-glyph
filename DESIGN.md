...
- **常用註記快速鍵**（欄位下方一排 chip，點一下插進游標處）：
  `待查`／`待確認`／`疑誤植：`／`出處：`／`依字形判定：`／`與「」易混`（游標落在「」裡）／`異體：`／`〔今天日期〕`。
  - ⚠️ **清單是「資料」不是 UI 文案**（§6.1 兩條通道，同 `app-launcher` 的 registry）⇒ **刻意不進 `locales`、不翻譯**：
    註記是策劃者用自己的語言寫的**內容**，不是介面文字。要改就改 `rare-glyph.js` 的 `NOTE_SNIPPETS` 一個陣列。
  - ⚠️ **這幾條不是憑空想的，也不是「常用」的證據**——備註欄剛上線、活資料 0 筆有備註。
    它們抓的是 owner 在**同領域**實際寫過的東西裡反覆出現的**形狀**
    （`db_siddham` 的 `tb_glyph_correction` 17 筆理由 ＋ `tb_glyph_review_flag` 4 筆：
    判定依據「依…字形判定」「由『X』字體佐證」／誤植／待確認／易混／異體）。
    **真正的「常用」要等用一陣子才知道，所以這個陣列本來就是要被改的。**
  - **日期戳記**存在的理由：備註**沒有 audit 欄位**，要記「這句是什麼時候寫的」只能寫進內文。
...
**聲調不分**（2026-09-19）：`juan` 也要搜得到 `juàn`——鍵盤打不出 `àáǎ`，而使用者不該為此背聲調。
`RareGlyphLib.foldTones()` 折掉聲調，**查詢與被查的字串兩邊都折**（只折一邊等於換一種方式漏）：

- **拼音的聲調是組合記號**（`á` = `a` + U+0301）⇒ `NFD` 拆開後去掉 U+0300–U+036F。
  同一段也含 `ü` 的兩點與 `ǹ ḿ`，⚠️ **一併去掉是刻意的**（`nu` 找得到 `nǚ`）。
- **注音的聲調是獨立字元**（`ˊ ˇ ˋ ˙` 與極少用的 `ˉ`）⇒ `NFD` 動不到，另外逐字去掉。

⚠️⚠️ **它是「追加的一條路」，不是取代原本的嚴格比對**：
`hay.indexOf(q) >= 0 || foldTones(hay).indexOf(foldTones(q)) >= 0`。
取代的話，**只打一個聲調符號**（`ˊ`，用來一次撈出所有二聲）會被折成空字串而**命中全部**。
折完是空的就不走寬鬆那條——實測 `ˊ` 仍是 4/65（`dú bá é fú`）、`ˋ` 4/65（`wà juàn sè wù`）、`ˇ` 0。
代價是**聲調打錯也會命中**（`juán` 找得到 `juàn`）——那是刻意的取捨：查不到比多幾筆糟。
...
- **find 的去聲調是「多一條路」不是「換一條路」**（2026-09-19）：換掉的話，「只打一個聲調符號」這個查法會從
  「撈出該聲調的字」變成「命中全部」——**一個原本有用的查詢悄悄變成沒有用的查詢，而畫面上只是結果變多**。
